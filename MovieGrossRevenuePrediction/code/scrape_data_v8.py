...
def getrottentomato(ttnumber,outputdata,omdbrequest):
	...
	tomatoe_link = 'empty'
	items = 10
	for link in duckduckgo.search(search, max_results=items):
		#print link
		if link.find('rottentomatoes.com/m/') != -1 :
			tomatoe_link = link
			break

	while tomatoe_link == 'empty':
		print 'heyy'
		items = items + 10
		for link in duckduckgo.search(search, max_results=items):
			if link.find('rottentomatoes.com/m/') != -1 :
	...
	outputdata[line]['tomatoMeter'] = tomatoRating.strip()
	print tomatoRating
