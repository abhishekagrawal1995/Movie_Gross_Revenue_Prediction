...
def main():
	...
	infilename = '../output_rotten/'+str(year)+'/movies_merge_'+str(year)+'.json'
	...
	for key, value in keys.iteritems():
		...
		gross_ = gross_.replace(',','')
		if gross_.find('CAD') != -1:
			#print key,budget_
			gross_ = gross_.replace('CAD', '').strip()
			gross_n = long(gross_) * 0.76
			gross_ = str(long(gross_n))
		#print gross_
		...
		if budget_.find('$CAD') != -1:
			...
			budget_ = str(long(budget_n))
		elif budget_.find('NOK') != -1 or budget_.find('SEK') != -1:
			#print key,budget_
			budget_ =  budget_.replace('NOK','').strip()
			budget_ =  budget_.replace('SEK','').strip()
			budget_n = long(budget_)*0.12
			budget_ = str(long(budget_n))
		...
		if keys[key]['tomatoMeter'] == 'empty':
			keys[key]['tomatoRating'] = 'empty'

		tmeter_ = keys[key]['tomatoMeter']
		tmeter_ = tmeter_.replace('%','')
		keys[key]['tomatoMeter'] = tmeter_
		umeter_ = keys[key]['userMeter']
		umeter_ = umeter_.replace('%','')
		keys[key]['userMeter'] = umeter_
		ureview_ = keys[key]['userreviews']
		ureview_ = ureview_.replace(',','')
		keys[key]['userreviews'] = ureview_
